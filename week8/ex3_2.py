...
def mandelbrot_escape_time_multiple(array, limits, i):
...
def generate_mandelbrot_set_chunks(array, limits, num_processes):
...
if __name__ == "__main__":
    ...
    xmin, xmax = -2, 2
    ymin, ymax = -2, 2
    chunks = sys.argv[2] if len(sys.argv) > 2 else 400
    num_proc = int(sys.argv[3]) if len(sys.argv) > 3 else 3
    fname = sys.argv[4] if len(sys.argv) > 4 else 'mandelbrot.zarr'
